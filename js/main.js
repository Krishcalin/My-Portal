  ...
  grid.innerHTML = TOOLS.map(tool => {
    ...
    return `<article class="tool-card" data-id="${tool.id}" data-category="${tool.category}" data-tags="${tool.tags.join(' ').toLowerCase()} ${tool.name.toLowerCase()} ${tool.tagline.toLowerCase()} ${tool.language.toLowerCase()}">
      ...
      <div class="tool-card__actions">
        ...
        ${tool.sampleReport
          ? `<a href="${tool.sampleReport}" class="btn btn--ghost btn--sm" target="_blank" rel="noopener" style="border-color:var(--accent-1);color:var(--accent-1)">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
              Sample Report
            </a>`
          : tool.overview
            ? `<a href="${tool.overview}" class="btn btn--ghost btn--sm" target="_blank" rel="noopener" style="border-color:var(--accent-2);color:var(--accent-2)">
