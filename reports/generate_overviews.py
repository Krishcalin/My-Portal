...
Generates 19 professional "Tool Overview" HTML pages for non-scanner tools
...
TOOLS = [
    ...
    {
        "filename": "ai-runtime-guardrail-overview.html",
        "title_plain": "AI Runtime Guardrail",
        "title_h1": ("AI Runtime ", "Guardrail"),
        "description": "A defensive, provider-agnostic real-time inspection proxy that sits between applications and any LLM API (Claude, OpenAI-compatible, self-hosted). It enforces security policy on inputs (prompt injection, jailbreak, PII/secret leakage) and outputs (data exfiltration, system-prompt leakage) using a cheapest-first detection stack — regex signatures, then heuristics, then a gated Claude LLM-judge. Verdicts drive block / redact / alert / allow actions, every decision is written to an immutable JSONL audit chain, and detections map to OWASP LLM Top 10, NIST AI RMF and MITRE ATLAS for compliance. Ships with an adversarial benchmark harness and CI recall/FPR gate. This is an early-stage (v0.1.0) but fully phase-complete build.",
        ...
        "highlights": [
            "Layered detection: 13 injection signatures + heuristic obfuscation checks + gated Claude Haiku LLM-judge backstop",
            "Inline FastAPI reverse proxy, drop-in via LLM base-URL swap, with Anthropic + OpenAI-compatible provider adapters",
            "Input & output redaction masks secrets before they reach the upstream and before completions return",
            "YAML-driven rules and policies — signatures, DLP patterns and per-category action overrides need no code change",
            "Safe-by-default: monitor mode never blocks, inline enforcement is opt-in, the LLM-judge is off unless enabled and fails open",
            "Adversarial benchmark (recall / FPR / F1 / latency, per-technique) usable as a CI regression gate",
            "OWASP LLM / NIST AI RMF / MITRE ATLAS control mapping with HTML/JSON/CSV GRC reports",
            "Immutable JSONL audit chain plus SIEM alerting (JSONL file and/or webhook)",
        ],
        "categories": [
        ...
        "compliance": [
            ("OWASP LLM Top 10 (2025)", "Each detection tagged to LLM01 Prompt Injection, LLM02 Sensitive Information Disclosure and LLM07 System Prompt Leakage"),
            ("NIST AI RMF", "Category findings mapped to Govern/Map/Measure/Manage subcategories for audit-ready coverage"),
            ("MITRE ATLAS", "Detections mapped to ATLAS techniques (AML.T0051 Prompt Injection, AML.T0057 Data Leakage, AML.T0056 Meta Prompt Extraction)"),
        ],
        "language": "Python",
        "components": [
            "core/",
    ...
    {
        "filename": "sbom-security-overview.html",
        "title_plain": "SBOM Security",
        "title_h1": ("SBOM ", "Security"),
        "description": "SBOM Security generates CycloneDX 1.5 and SPDX 2.3 software bills of materials from project lockfiles and manifests across Python, Node, Maven, and Go, correlates each component with known vulnerabilities from OSV.dev, tracks dependency drift against a baseline snapshot, and enforces license and security policy in CI. Every component is keyed to a spec-correct package URL (purl) so vulnerability lookup, drift detection, and deduplication stay interoperable. Combined JSON/CSV/HTML reports roll up findings against NTIA, CISA, OWASP SCVS, and NIST SSDF frameworks. Development is organized into six completed phases, covered by 55 pytest tests.",
        ...
        "highlights": [
            "CycloneDX 1.5 and SPDX 2.3 output with per-component purl, licenses, and properties",
            "OSV.dev vulnerability correlation with CVSS, severity, and fixed-in version, plus --fail-on CI gate",
            "Dependency drift and baseline snapshots reporting added/removed/upgraded/downgraded components",
            "YAML policy engine for license allow/deny, banned packages, and max-severity vulnerability gates",
            "Combined JSON/CSV/HTML supply-chain reports with GRC compliance roll-up",
            "Pre-commit hooks and GitHub Actions workflow (Python 3.10-3.12 matrix) for CI enforcement",
            "Stdlib-first parsing with 55 offline pytest tests and no live network calls in the suite",
        ],
        "categories": [
            "SBOM Generation",
            "CycloneDX",
        ...
        "compliance": [
            ("NTIA Minimum Elements", "Scores the generated SBOM against the 2021 NTIA baseline minimum elements."),
            ("CISA SBOM", "Maps CISA SBOM guidance built on the NTIA minimum elements."),
            ("OWASP SCVS", "Maps Software Component Verification Standard controls to SBOM findings."),
            ("NIST SSDF (SP 800-218)", "Maps secure-software-development practices for third-party components."),
        ],
        "language": "Python 3.10+",
        "components": [
            "main.py",
    ...
    {
        "filename": "secrets-overview.html",
        "title_plain": "Secrets Scanner",
        "title_h1": ("Secrets ", "Scanner"),
        "description": "A defensive secrets scanner that detects hardcoded credentials across the working tree, config files, and full git history. Detection is read-only and redacted by design — every finding stores a masked preview and a stable fingerprint, never the raw value. It ships 28 YAML-defined signature rules with per-rule entropy gating, an opt-in entropy detector for novel formats, three-layer noise control (allowlist, baseline, inline pragmas), safe opt-in live verification against 6 providers, and JSON/CSV/HTML reports with CWE/OWASP/PCI-DSS/NIST compliance mapping. A single --fail-on flag turns it into a CI build gate.",
        ...
        "highlights": [
            "28 signature rules (16 critical / 9 high / 3 medium) covering AWS, Azure, GCP, GitHub, GitLab, OpenAI, Anthropic, Stripe, Slack, Twilio, npm, PyPI, PEM keys, JWTs, and DB URIs",
            "Rules are data: every pattern lives in rules/secret_patterns.yaml and can be added or tuned with no Python change",
            "Per-rule Shannon-entropy gating plus an opt-in entropy detector to flag unknown high-entropy key formats",
            "Git-history command reconstructs each commit's added lines and attributes findings to the introducing commit/author",
            "Safe, opt-in live verification: read-only, provider-scoped, no-redirect, rate-limited, fail-safe to UNVERIFIED, raw secret never stored",
            "Compliance mapping ties every finding to CWE-798/321/522, OWASP Top 10 2021, PCI DSS v4.0, and NIST 800-53 controls",
            "Pre-commit hook and GitHub Actions gate that fails only on newly introduced secrets via a committed baseline",
        ],
        "categories": [
            "Cloud credentials (AWS/Azure/GCP)",
            "VCS & CI tokens (GitHub/GitLab)",
            "AI/LLM API keys (OpenAI/Anthropic)",
            "SaaS API keys (Stripe/Twilio/Slack)",
            "Package registry tokens (npm/PyPI)",
            "Private keys, JWTs & DB URIs",
            "Entropy-based detection",
            "Git-history scanning",
            "Live credential verification",
            "Baseline & allowlist triage",
            "CI / pre-commit gating",
            "Compliance mapping",
        ],
        "compliance": [
            ("CWE-798 / 321 / 522", "Use of hardcoded credentials, hardcoded cryptographic keys, and insufficiently protected credentials"),
            ("OWASP Top 10 2021", "A02 Cryptographic Failures and A07 Identification & Authentication Failures"),
            ("PCI DSS v4.0", "Requirements 8.6.2 (no hardcoded credentials) and 6.2.4 secure development"),
            ("NIST SP 800-53 Rev 5", "IA-5 Authenticator Management and SC-12 Cryptographic Key Management"),
        ],
        "language": "Python",
        "components": [
            "main.py",
    ...
    {
        "filename": "ad-assess-overview.html",
        "title_plain": "Active Directory Security Assessment",
        "title_h1": ("Active Directory ", "Security Assessment"),
        "description": "A defensive, read-only security assessment tool for Microsoft Active Directory. It connects over LDAP/LDAPS, and optionally WinRM, to run 86 checks across 15 categories including Kerberos attacks, AD CS certificate abuse, privilege escalation, password policy, trust relationships, and authentication weaknesses. It produces a self-contained HTML report and a JSON export with 0-100 posture scoring, A-F grading, and per-finding PowerShell remediation commands. All LDAP queries are search-only and all WinRM commands are Get-*, so the scanner never modifies AD objects.",
        ...
        "highlights": [
            "86 distinct security checks across 15 modules — Kerberos attacks, AD CS certificate abuse, privileged accounts, password policy, trusts, and authentication weaknesses",
            "Read-only assessment: LDAP search operations and PowerShell Get-* cmdlets only, never Set-/New-/Remove- operations against the directory.",
            "Optional WinRM path adds registry and audit-policy checks (SMB signing, WDigest, Credential Guard, NTLM), skipped with INFO notices when unreachable.",
            "Posture scoring and A-F grading computed in utils/severity.py, with severity-based exit codes for pipeline gating.",
            "Interactive, self-contained HTML report (inline CSS/JS, dark theme, severity filters, live search) alongside a structured JSON report.",
            "15 independent check modules are individually selectable via --checks or a YAML scan profile (config/default_profile.yaml).",
        ],
        "categories": [
            "Kerberos Attacks",
            "AD CS (ESC1/2/6/8)",
        ...
        "compliance": [
            ("CIS Microsoft AD Benchmark", "Referenced as the hardening basis for password-length and configuration guidance in check recommendations (password_policy.py) and CLAUDE.md conventions."),
            ("MITRE ATT&CK", "Listed as a compliance-mapping reference in CLAUDE.md conventions; covered adversary techniques include Kerberoasting, AS-REP roasting, DCSync, and NTLM relay (no formal per-check technique IDs shipped)."),
            ("ANSSI AD Security Guide", "French ANSSI Active Directory hardening guidance listed as a compliance-mapping reference in CLAUDE.md."),
        ],
        "language": "Python",
        "components": [
            "ad_security_scanner.py",
            "checks/",
            "utils/",
    ...
    {
        "filename": "alpha-ai-overview.html",
        "title_plain": "Alpha-AI",
        "title_h1": ("Agentic AI ", "Red-Team Framework"),
        "description": "Alpha-AI is an early-stage (v0.1.0, Alpha) agentic offensive-security framework that exposes command-line pentest tools to LLM agents (Claude/GPT/Copilot) over two parallel surfaces: a FastMCP stdio server and a FastAPI REST API, both backed by a single tool registry. Nine tools spanning recon, web, network, credential, and exploit categories (nmap, nuclei, gobuster, ffuf, sqlmap, enum4linux, crackmapexec, hydra, searchsploit) each register a thin async wrapper that builds argv, runs it through a timeout-aware CommandRunner, and feeds stdout to a pure parser that emits normalized Finding objects. A dispatcher enforces the safety model on every call: a target-authorization whitelist (literals, CIDR ranges, globs) blocks non-authorized hosts before execution, and a disk cache keyed on sha256(tool, target, args) prevents redundant scans. Designed to run inside a Kali Linux container. Maturity is genuinely early: 4 commits, 24 unit tests, and no CI pipeline. A threat_intel module is present but uses mock data and is not yet wired into the servers. Authorized security testing only.",
        "github": "https://github.com/Krishcalin/Alpha-AI",
        "stats": [
            ("Security Tools Wrapped", "9"),
            ("Unit Tests", "24"),
            ("Python LOC", "~2,900"),
            ("Version / Maturity", "0.1.0 (Alpha)"),
        ],
        "highlights": [
            "Single-source-of-truth ToolRegistry: MCP shims and REST routes both call one shared dispatcher, so authorization, caching, and logging are enforced in exactly one place.",
            "Authorization-by-default: all tools require a whitelisted target; only local-only searchsploit opts out via requires_authorization=False.",
            "Severity policies per tool are explicit (e.g. sqlmap injection and hydra valid creds are always CRITICAL; crackmapexec Pwn3d! is CRITICAL; nmap open ports are INFO).",
            "Async subprocess runner with timeout, env isolation, which()-based binary discovery, and structlog-structured audit logging of argv/returncode/duration.",
            "Pydantic models (Target, Finding, CommandResult, ToolResult, Severity, ToolCategory) give a typed contract shared across both server surfaces.",
            "Documented extension pattern (parser + wrapper + registry hook + MCP shim + REST route + parser test) and a Phase 2-5 roadmap for tool breadth, orchestration, reporting, and CI.",
            "Early-stage honesty: 4 commits, no CI/CD, and a mock-data threat_intel module not yet connected to the tool surfaces.",
        ],
        "categories": [
            "Reconnaissance",
        ...
        "compliance": [
            ("MITRE ATT&CK", "Technique-mapping model exists in the threat_intel module and per-finding ATT&CK mapping is on the Phase 4 roadmap; not yet wired into live tool output."),
            ("CVE", "Finding objects carry a CVE list; the nuclei parser tags results with CVE identifiers from template metadata."),
            ("Exploit-DB", "The searchsploit wrapper queries the local Exploit-DB copy and classifies entries (webapps/remote to HIGH, local/dos to MEDIUM)."),
            ("Authorized-Use / Rules of Engagement", "Framework blocks any remote tool run against non-whitelisted targets and is documented as authorized security testing only."),
        ],
        "language": "Python",
        "components": [
            "alpha_ai/",
